...
=for stopwords invocant invocants lexicals unintuitive yada globals ro rw boolification
...
For a function which only returns one value:

   fun foo ($a → Int) { ... }

For a function which returns a list in list context:
...
B<TODO:>
There are some minor bugs parsing multiple return type constraints if
they don't have traits (C<scalar>, C<list> and C<coerce> are traits of
course!).

B<TODO:>
Setting the C<coerce> trait on one return type constraint automatically
applies it to all return type constraints in that signature. (So you
can't, say, coerce in scalar context without coercing in list context
too.)
